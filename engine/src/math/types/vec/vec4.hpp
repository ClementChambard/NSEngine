...
struct vec4 {
  ...
  vec4& operator=(vec2 const& other) { x = other.x, y = other.y, z = 0.0f, w = 0.0f; return *this; }
  vec4& operator=(vec3 const& other) { x = other.x, y = other.y, z = other.z, w = 0.0f; return *this; }
  ...
  bool operator==(vec4 const &other) const {
    if (abs(x - other.x) > NS_FEPSILON)
      return false;
    if (abs(y - other.y) > NS_FEPSILON)
      return false;
    if (abs(z - other.z) > NS_FEPSILON)
      return false;
    if (abs(w - other.w) > NS_FEPSILON)
      return false;
    return true;
  }
