...
void MouseStruct::SetPos(i32 x, i32 y)
{
    auto windata = getInstance()->window().getWindowData();
    float displayRatio = windata.width/(float)windata.bwidth;
    SDL_WarpMouseInWindow(getInstance()->window().getSdlWindow(), (i32)((i32)(x+windata.bwidth/2)/displayRatio), (i32)((i32)(-y+windata.bheight/2)/displayRatio));
}
...
void Inputs::InitGamepads()
{
    ...
    for (i32 i = 0; i < numJoystick; i++)
    {
        ...
        if (SDL_IsGameController(i)) {
               ...
               ns::info("opened controller : ", SDL_GameControllerName(c));
        } else ns::info("opened joystick");
