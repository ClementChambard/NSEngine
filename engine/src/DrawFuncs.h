...
/** batch_draw_AA_box(batch, pos1, pos2, color, outline)
 * 
 * draws a 3d axis aligned box from pos1 to pos2 with color color
 * \param batch:   Where to draw the AA box.
 * \param p1:      First coordinate of the box.
 * \param p2:      Second coordinate of the box.
 * \param c:       Color of the box.
 * \param shade:   if enabled, shades the sides for pseudo lighting effect
 */
void batch_draw_aa_box(DrawBatch* batch, vec3 p1, vec3 p2, Color c, bool shade = true);
