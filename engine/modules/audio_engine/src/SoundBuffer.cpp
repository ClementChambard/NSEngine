...
ALuint add_sound_effect(const char* filename)
{
    ...
    if (!sndfile) { NS_ERROR("Could not open audio in %s: %s", filename, sf_strerror(sndfile)); return 0; }
    if (sfinfo.frames < 1 || sfinfo.frames > (sf_count_t)(INT32_MAX / sizeof(short)) / sfinfo.channels)
    { NS_ERROR("Bad sample count in %s: %s", filename, sfinfo.frames); sf_close(sndfile); return 0; }
    format = AL_NONE;
    ...
    if (!format)
    { NS_ERROR("Unsupperted channel count : %d", sfinfo.channels); sf_close(sndfile); return 0; }
    ...
    if (num_frames < 1)
    { ns::free_raw(membuff, (size_t)(sfinfo.frames * sfinfo.channels) * sizeof(short)); sf_close(sndfile); NS_ERROR("Failed to read samples in %s: %d", filename, num_frames); return 0; }
