...
struct mat4 {
  ...
  static mat4 perspective(f32 fov_rad, f32 aspect_ratio, f32 near_clip, f32 far_clip) {
    ...
    out_matrix[2][2] = (near_clip + far_clip) * fn;
    out_matrix[2][3] = -1.0f;
    out_matrix[3][2] = -2 * far_clip * near_clip * fn;
    return out_matrix;
  }

  static mat4 perspective_lh(f32 fov_rad, f32 aspect_ratio, f32 near_clip, f32 far_clip) {
    f32 half_tan_fov = tan(fov_rad * 0.5f);
    mat4 out_matrix(0);
    f32 fn = 1.0f / (far_clip - near_clip);
    out_matrix[0][0] = 1.0f / (aspect_ratio * half_tan_fov);
  ...
  static mat4 lookat(vec3 const &position, vec3 const &target, vec3 const &up) {
	...
	static mat4 lookat_lh(vec3 const &position, vec3 const &target, vec3 const &up) {
		mat4 out_matrix(1.0f);
	  vec3 const f = (target - position).normalized();
	  ...
	  vec3 const u = f.cross(s);
		...
		out_matrix[2][1] = u.z;
		out_matrix[2][2] = f.z;
		out_matrix[3][0] = -s.dot(position);
		out_matrix[3][1] = -u.dot(position);
		out_matrix[3][2] = -f.dot(position);
		return out_matrix;
	}
  ...
  static mat4 mk_rotate(f32 angle, ns::vec3 const& axis) {
    ...
    out_matrix[0][0] = oc * axis_n.x * axis_n.x + c;
    out_matrix[0][1] = oc * axis_n.x * axis_n.y - axis_n.z * s;
    out_matrix[0][2] = oc * axis_n.z * axis_n.x + axis_n.y * s;
    out_matrix[1][0] = oc * axis_n.x * axis_n.y + axis_n.z * s;
    out_matrix[1][1] = oc * axis_n.y * axis_n.y + c;
    out_matrix[1][2] = oc * axis_n.y * axis_n.z - axis_n.x * s;
    out_matrix[2][0] = oc * axis_n.z * axis_n.x - axis_n.y * s;
    out_matrix[2][1] = oc * axis_n.y * axis_n.z + axis_n.x * s;
    out_matrix[2][2] = oc * axis_n.z * axis_n.z + c;
